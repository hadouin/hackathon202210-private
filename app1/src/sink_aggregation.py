...
###
# This Sink service transform incoming Wi-Fi data from one station (PC,Smartphone, IoT Device, Sound device ...)
# into aggregate data and detect anomalies to improve Wi-Fi services
# Example message:
# {
#  "info": {
#        "identifier": "GTW02",
...
# - Wi-Fi metrics aggregation (min/max/avg)
# - Number of connection changes during the data collection
# - Monitoring RSSI value during the collection
# - RSSI (Received Signal Strength Indication) --> Indication of Wi-Fi Link quality
#                                              --> A low value indicate a poor Wi-Fi link quality
...
def sink_aggregation(json_data):
    aggregate_data = {"identifier": json_data["info"]["identifier"],
                      ...
                      "endTime": find_max(json_data["wifiData"], "eventTime"), "wifiAggregate": {
        ...
        }, "anomalies_report": detect_anomaly_min(json_data["wifiData"], "rssi", RSSI_THRESHOLD)}
...
def find_min(array, key):
    if len(array) > 0:
        ...
        for i in range(len(array)):
            if array[i][key] is not None:
                if array[i][key] < minimum:
                    minimum = array[i][key]
        return minimum
    return None
    ...
    if len(array) > 0:
        for i in range(len(array)):
            if array[i][key] < threshold:
                anomaly_report = {"eventTime": array[i]["eventTime"], "deviceType": array[i]["deviceType"],
                                  "rssi": array[i]["rssi"], "connection": array[i]["connection"]}
                array_anomaly.append(anomaly_report)
    return array_anomaly


# Connect on mongodb and post one aggregated Wi-Fi Data
...
def find_data_mongo(mongo_server: MongoClient, identifier: str):
    try:
        return mongo_server[MONGO_DB_DATABASE_NAME][MONGO_DB_COLLECTION_NAME]\
